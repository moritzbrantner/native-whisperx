...
| Rust-Native benchmark ladder | 30s, 3m, and 10m large-v3-turbo CUDA clips | `blocked` | The ladder remains a manual/report-only throughput check in `tests/parity/rust-native-bench-fixtures.json`, not a `final-full-surface` merge gate. Master validation on 2026-06-20 passed the 30s and 3m rungs but failed the 10m rung because native ASR is still sequential over chunks (`batchExecution=candle-whisper-sequential`), with 10m native ASR taking about 43.2-43.6s versus WhisperX total time around 21.0-22.4s. Runtime follow-up work is tracked in #34, #35, #36, and #37. |
