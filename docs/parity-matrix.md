...
| Decode controls | temperature, beam/best-of, patience, penalties, suppression, prompts, fp16, thresholds, threads | `rust-native complete` | Native maps temperature schedules, best-of sampling, beam size, patience, length penalty, prompt seeding, explicit token and numeral suppression, request-local previous-text conditioning, fallback thresholds, silence rejection, and positive request-scoped decoder threads; zero threads fail before model setup. WhisperX `--fp16` remains intentionally unsupported in favor of native compute type, and hotwords remain intentionally unsupported as a faster-whisper-specific control. |
