...
| Performance benchmark | blocked by runtime work | `parity-bench` JSON report | Use `native-whisperx parity-bench` for native-vs-WhisperX elapsed time, realtime factor, diagnostics, and batch-path reporting. The benchmark remains manual/report-only while 10m native ASR is slower than WhisperX and native chunk execution is still sequential. |
| Rust-Native benchmark ladder | blocked by runtime work | `tests/parity/rust-native-bench-fixtures.json` manual report | 30s and 3m large-v3-turbo CUDA rungs beat WhisperX in master validation, but the 10m rung fails. Native total time is about 51s, with about 43s in ASR, versus WhisperX total time around 21-22s. Diagnostics show `chunkCount=20`, `batchCount=3`, and `batchExecution=candle-whisper-sequential`; real native ASR batching/runtime optimization is required before this row can complete. |
