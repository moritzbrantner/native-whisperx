...
| Decode controls | native complete | public runtime/decode coverage, unit rejection coverage, and ignored native ASR smokes | Native maps temperature schedules, best-of, beam size, patience, length penalty, prompt seeding, explicit token and numeral suppression, request-local previous-text conditioning, fallback thresholds, silence rejection, and positive `--threads`; zero threads fail before model setup. WhisperX `--fp16` remains intentionally unsupported in favor of native compute type, while hotwords remain intentionally unsupported. |
